...
impl Preposition {
  fn new(base_form: &str, case: PrepositionCase) -> Preposition {
    Preposition {
      base_form: base_form.to_string(),
      case,
    }
  }  
  fn get_case(&self) -> PrepositionCase {
    self.case
  }
}
...
impl Interjection {
  fn new(word: &str, sentiment: Sentiment) -> Interjection {
    Interjection {
    word: word.to_string(),
    sentiment,
    }
